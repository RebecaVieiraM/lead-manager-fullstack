...
export default function LeadsDeclined({ leads }) {
  ...
  if (leads.length === 0) return <p className="no-leads">Nenhum lead recusado no momento.</p>;

  return (
    <div>
      {leads.map(lead => (
        <div key={lead.id} className="lead-card declined">
          ...
          <p><b>Preço:</b> R$ {lead.price.toFixed(2)}</p>
